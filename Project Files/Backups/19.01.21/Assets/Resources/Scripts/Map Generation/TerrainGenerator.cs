...
public class TerrainGenerator : MonoBehaviour
{
	...
	public static bool CanAddToNearestChunk(Vector3 t)
	{
		...
		print(instance.terrainChunkDictionary[chunkLoc].meshObject.transform.name);

		if (instance.terrainChunkDictionary[chunkLoc].meshObject.transform.childCount > 0) { Debug.Log("true"); return true; } else { Debug.Log("false"); return false; }
	}
	...
	// Called to spawn a chunk from server data
	public void SpawnChunk(Vector2 coord, Dictionary<string, Vector3> props) 
    {
		// Check if the chunk already exists
		if (!multiplayerChunkDictionary.ContainsKey(coord))
        {
			// Create a new chunk
			MultiplayerTerrainChunk newChunk = new MultiplayerTerrainChunk(coord, heightMapSettings, meshSettings, transform, viewer, mapMaterial);
			// Load the chunk
			newChunk.Load();
			// Add the chunk to the chunk dict
			multiplayerChunkDictionary.Add(coord, newChunk);
		}
    }
