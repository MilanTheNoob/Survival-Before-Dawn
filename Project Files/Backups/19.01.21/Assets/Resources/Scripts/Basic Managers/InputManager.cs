...
public class InputManager : MonoBehaviour
{
    ...
    public static bool onGameLoaded;
    ...
    {        
        ...
        interactionInputSettings.ResetInput();
    }
    ...
    public void SavePos() { print("tried to save"); SavingManager.SaveFile.playerPos = player.transform.position; }
    ...
    public void DisableUISections()
    {
        ...
        {
            if (UISections[i].activeSelf) { TweeningLibrary.FadeOut(UISections[i], 0.7f); }
        }
    }
    ...
    public void ToggleUISectionsInt(int UIIndex)
    {
        for (int i = 0; i < UISections.Length; i++)
        {
            if (i == UIIndex)
            {
                TweeningLibrary.FadeIn(UISections[i], 0.7f);
            }
            else if (UISections[i].activeSelf)
            {
                TweeningLibrary.FadeOut(UISections[i], 0.7f);
            }
        }
    }
